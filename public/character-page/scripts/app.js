...
const initialCharacterId = (urlParamId && characters[urlParamId]) ? urlParamId : (characters[DEFAULT_CHARACTER_ID] ? DEFAULT_CHARACTER_ID : characterIds[0]);
...
// ── BGM 크로스페이드 매니저 ─────────────────────────────────
const BGM = (() => {
  const FADE_MS = 1200;   // 크로스페이드 시간
  const VOLUME = 0.35;    // 기본 볼륨

  let nodeA = null;       // 현재 재생 중인 Audio
  let nodeB = null;       // 다음 재생할 Audio
  let currentSrc = null;  // 현재 재생 중인 src
  let pendingId = 0;      // 악의적 연속 호출 방어용 ID
  let isMuted = false;

  function _createNode(src) {
  ...
  function _fadeTo(node, target, ms) {
    ...
    const step = (target - start) / (ms / 16);
    const tick = () => {
      node.volume = Math.max(0, Math.min(1, node.volume + step));
      if (Math.abs(node.volume - target) > Math.abs(step) * 0.5) {
        requestAnimationFrame(tick);
      } else {
        node.volume = target;
        if (target === 0) { node.pause(); node.src = ''; }
      }
    };
    requestAnimationFrame(tick);
  }

  function play(src, force = false) {
    if (!src) return;
    ...
    if (nodeA) { _fadeTo(nodeA, 0, FADE_MS); }

    if (isMuted) return; // 뮤트 상태면 src만 기억해두고 재생하지 않음

    const next = _createNode(src);
    nodeB = next;
    next.play().then(() => {
      if (pendingId !== myId || isMuted) {
        next.pause();
        next.src = '';
        return;
      }
      _fadeTo(next, VOLUME, FADE_MS);
      nodeA = next;
      nodeB = null;
    }).catch(() => { });
  }
  ...
  function stop() {
    if (nodeA) { _fadeTo(nodeA, 0, FADE_MS); nodeA = null; }
    currentSrc = null;
  }
  ...
  function toggle() {
    ...
    if (isMuted) {
      ...
      if (currentSrc) play(currentSrc, true);
    }
    return isMuted;
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (bgmBtn) {
    const iconOn = bgmBtn.querySelector('.bgm-icon--on');
    const iconOff = bgmBtn.querySelector('.bgm-icon--off');
    bgmBtn.addEventListener('click', () => {
      const isMuted = BGM.toggle();
      bgmBtn.setAttribute('aria-pressed', !isMuted ? 'true' : 'false');
...
function animateParticles() {
  ...
  particles.forEach((particle) => {
    ...
    particleCtx.fillStyle = `rgba(${particleColor[0]}, ${particleColor[1]}, ${particleColor[2]}, ${alpha * particle.depth
      })`;
    ...
    if (speedAbs > 0.3) {
      particleCtx.strokeStyle = `rgba(${particleColor[0]}, ${particleColor[1]}, ${particleColor[2]}, ${0.08 * particle.depth * speedAbs
        })`;
...
// ── ASCII ART 펼치기 토글 ────────────────────────────
function createClickParticles(x, y, colorStr) {
  const count = 12;
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (colorStr) {
      p.style.background = `rgb(${colorStr})`;
      p.style.boxShadow = `0 0 8px rgba(${colorStr}, 0.6)`;
    }
...
if (asciiToggleBtn && snapshotTerminalDock) {
  asciiToggleBtn.addEventListener('click', (e) => {
    // 클릭 이펙트
    const rect = asciiToggleBtn.getBoundingClientRect();
    const x = e.clientX || rect.left + rect.width / 2;
    const y = e.clientY || rect.top + rect.height / 2;
    createClickParticles(x, y, particleColor);

    const isExpanded = asciiToggleBtn.getAttribute('aria-expanded') === 'true';
    if (isExpanded) {
      snapshotTerminalDock.hidden = true;
      asciiToggleBtn.setAttribute('aria-expanded', 'false');
    } else {
      snapshotTerminalDock.hidden = false;
      asciiToggleBtn.setAttribute('aria-expanded', 'true');
      // 펼칠 때 ASCII 렌더링이 뷰포트 크기를 인식하도록 리사이즈 트리거
      window.setTimeout(() => terminalExperience.handleResize(), 50);
    }
  });
}
